...
const triggers = ['All hooks', 'Pre token creation', 'Post token creation', 'Pre token deletion', 'Post token deletion'];
...
async function _renderDialog() {
	...
	for (const trigger of triggers) {
		...
		if (trigger === 'All hooks') content += '<hr>';
		index++;
	}
	const choices = await foundry.applications.api.DialogV2.prompt({
		...
		render: (event, dialog) => {
			dialog.element.querySelectorAll("input[type='text'][data-trigger]").forEach((el) => {
				el.addEventListener('drop', (ev) => _onDrop(ev, el.dataset.trigger));
			});
			const allHooksInput = dialog.element.querySelector("input[type='text'][data-trigger='all-hooks']");
			if (allHooksInput) {
				allHooksInput.addEventListener('input', (ev) => {
					if (ev.target.value === '') {
						dialog.element.querySelectorAll("input[type='text'][data-trigger]").forEach((el) => (el.value = ''));
					}
				});
			}
		},
